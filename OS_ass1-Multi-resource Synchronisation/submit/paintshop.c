...
struct semaphore *tint_access;
struct semaphore *tints_mutex[NCOLOURS];    //semaphore for each tint
...
void fill_order(void *v)
{
  ...
  // lock access to only requested tints
  int i;
  unsigned int color;
  struct paintcan *can = (struct paintcan *)v;
  
  P(tint_access);
  for(i = 0; i<PAINT_COMPLEXITY; i++)
  {
    color = can->requested_colours[i];
    if(color >0)
    {
      int j = i-1, flag = 0;
      while(j>=0)
      {
        //check if the thread already acquired this tint
        if(color == can->requested_colours[j]) flag =1;
        j--;
      }
     if(flag == 0) P(tints_mutex[color-1]);
    }
  }
  V(tint_access);
  ...
  for(i = 0; i<PAINT_COMPLEXITY; i++)
  {
    ...
    if(color >0)
    {
      ...
      while(j>=0)
      {
        //check if the thread already released this tint
        if(color == can->requested_colours[j]) flag =1;
        j--;
      }
     if(flag == 0) V(tints_mutex[color-1]);
    }
  }
  V(tint_access);
}
...
void paintshop_open()
{
  ...
  tint_access = sem_create("tint_access", 1);

  for(i =0 ; i< NCOLOURS; i++)
  {
    tints_mutex[i] = sem_create("tint_mutex", 1);
  }
...
void paintshop_close()
{
  ...
  sem_destroy(tint_access);
  int i;
  for(i =0 ; i< NCOLOURS; i++)
  {
    sem_destroy(tints_mutex[i]);
  }
}
